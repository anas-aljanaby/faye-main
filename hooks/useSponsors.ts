...
export const useSponsorsBasic = () => {
  ...
  const fetchSponsors = async (useCache = true) => {
    ...
    const cacheKey = `sponsors-basic:${userProfile.organization_id}`;
    ...
    try {
      ...
      // Fetch only count of sponsored orphans (lighter than fetching all relationships)
      const { data: allSponsorOrphans } = await supabase
        .from('sponsor_orphans')
        .select('sponsor_id, orphan_id')
        .in('sponsor_id', sponsorIds);

      // Group orphan IDs by sponsor (but we only need the count)
      const orphansBySponsor = new Map<string, string[]>();
      (allSponsorOrphans || []).forEach(so => {
        if (!orphansBySponsor.has(so.sponsor_id)) {
          orphansBySponsor.set(so.sponsor_id, []);
      ...
      // Build sponsors with minimal data
      const basicSponsors = sponsorsData.map((sponsor) => {
        const sponsoredOrphanIds = (orphansBySponsor.get(sponsor.id) || []).map(so => 
          uuidToNumber(so)
        );
        ...
        return {
          ...
          name: sponsor.name,
          avatarUrl: sponsor.avatar_url || '',
          sponsoredOrphanIds, // Keep for compatibility, but could be optimized further
        } as Sponsor;
      });

      setSponsors(basicSponsors);
      cache.set(cacheKey, basicSponsors, 5 * 60 * 1000);
    } catch (err) {
      console.error('Error fetching basic sponsors:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch sponsors');
    } finally {
      setLoading(false);
    }
