...
const navItems = [
    ...
    { to: '/payments', text: 'الدفعات', icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="10" rx="2"/><path d="M6 12h.01"/><path d="M10 12h.01"/><path d="M14 12h.01"/></svg> },
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
    ...
    // Sponsors should not see: sponsors, human-resources, financial-system
    // Team members should not see: payments (sponsor-only)
    const restrictedPaths = userProfile?.role === 'sponsor'
        ? ['/sponsors', '/human-resources', '/financial-system']
        : ['/payments'];
    const visibleNavItems = navItems.filter(item => !restrictedPaths.includes(item.to));
